...
const TimerGame: React.FC = () => {
  ...
  const [currentEvent, setCurrentEvent] = useState<EventType>("cs");
  ...
  const stopTimer = () => {
    ...
    // Send score to event-specific endpoint
    axiosHeader
      .post(`/timer/${currentEvent}/score`, {
        name,
        target: targetTime,
        actual: actualTime,
        difference: diff,
  ...
  const fetchLeaderboard = async () => {
    try {
      const response = await axiosHeader.get(
        `/timer/${currentEvent}/leaderboard`
      );
      ...
      >
        ...
        <div className="p-4 md:p-6 flex flex-col items-center w-full max-w-md mx-auto">
          ...
          <div className="text-center mb-6 md:mb-8">
            ...
            {/* Event Badge */}
            <div
              className={`text-xs font-bold py-1 px-3 rounded border-2 ${
                currentEvent === "cs"
                  ? "bg-[#0066ff] border-[#00ccff] text-white"
                  : "bg-[#ff00ff] border-[#ff66ff] text-white"
              }`}
              style={{
                fontFamily: '"Press Start 2P", monospace',
                textShadow:
